            ...
            {isLoading ? (
              ...
              items.map(item => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-xs text-muted-foreground truncate max-w-xs">{item.description}</TableCell>
                  <TableCell>{item.recipeName}</TableCell>
                  <TableCell className="text-right font-mono">${item.price.toFixed(2)}</TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow><TableCell colSpan={4} className="text-center h-24">{t('No menu items found.')}</TableCell></TableRow>
            )}
        </TableBody>
      </Table>
    </div>
