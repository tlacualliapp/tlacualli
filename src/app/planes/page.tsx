      ...
      <main>
        ...
        <section className="relative h-[60vh] min-h-[500px] w-full">
            ...
            <div className="relative z-10 flex h-full flex-col items-center justify-center text-center text-white p-4">
                <h1 className="font-headline text-5xl md:text-7xl font-extrabold tracking-tight">
                    Elige el plan que hará <span className="text-primary">crecer</span> tu negocio
                </h1>
                <p className="mt-6 max-w-3xl text-lg md:text-xl text-white/90 font-body">
                    En Tlacualli App, creemos que la tecnología debe ser accesible para todos. Por eso, hemos diseñado planes que se adaptan perfectamente al tamaño de tu restaurante, ayudándote a gestionar tu negocio de manera eficiente, sin complicaciones y a un precio justo.
                </p>
            </div>
        </section>
        ...
        <section className="py-12 md:py-16 bg-gray-50 dark:bg-gray-900/50">
            <div className="container text-center">
                 <h2 className="font-headline text-3xl md:text-4xl font-bold">Regístrate y prueba Tlacualli App gratis por 15 días</h2>
                 <p className="mt-4 max-w-3xl mx-auto text-muted-foreground font-body">
                    ¿Aún no estás seguro? No te preocupes. Te invitamos a probar todas las funcionalidades de Tlacualli App sin costo. Sin tarjeta de crédito ni compromisos. Explora cómo nuestra herramienta puede transformar la gestión de tu restaurante y descubre por qué miles de negocios confían en nosotros.
                 </p>
                    ...
                    {plans.map((plan, index) => (
                        <Card key={index} className={`flex flex-col ${plan.isPopular ? 'border-primary border-2 shadow-lg' : ''}`}>
                            ...
                            <CardHeader className="text-center">
                                ...
                                <p className="text-4xl font-bold font-headline pt-4">{plan.price.split(' ')[0]} <span className="text-lg font-normal text-muted-foreground">{plan.price.split(' ').slice(1).join(' ')}</span></p>
                            </CardHeader>
                            <CardContent className="flex-grow flex flex-col justify-between">
                                <div>
                                    <p className="text-sm font-body mb-6">{plan.salesText}</p>
        ...
        <section className="py-12 md:py-24 bg-gray-50 dark:bg-gray-900/50">
            <div className="container">
                 <div className="text-center mb-12">
                    <h2 className="font-headline text-3xl md:text-4xl font-bold">Compara nuestros planes</h2>
                    <p className="mt-4 max-w-2xl mx-auto text-muted-foreground font-body">Encuentra la solución perfecta para tu restaurante.</p>
                </div>
                <Card>
                    <Table>
                        <TableHeader>
                            <TableRow>
                                <TableHead className="w-1/4 font-headline text-lg">Funcionalidad</TableHead>
                                {plans.map(plan => <TableHead key={plan.name} className="text-center font-headline text-lg">{plan.name}</TableHead>)}
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            ...
                            <TableRow>
                                <TableCell className="font-bold">Precio mensual (+IVA)</TableCell>
                                {plans.map(plan => (
